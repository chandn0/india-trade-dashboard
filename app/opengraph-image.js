...
export const alt = 'India Trade Monitor — exports, imports and the trade balance from official Government of India data';
...
export default async function OpengraphImage() {
  return new ImageResponse(
    (
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 18, fontSize: 26, color: '#d9cbff', letterSpacing: 4 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ fontSize: 32, color: '#9aa6bd', lineHeight: 1.3, maxWidth: 920 }}>
            Year-wise totals, basket-level composition and partner shares — straight from official Government of India data.
          </div>
        </div>

        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 22, color: '#7a86a0' }}>
          <span>tradestat.commerce.gov.in &middot; RBI &middot; FBIL</span>
          <span style={{ color: '#d9cbff', fontWeight: 700, letterSpacing: 1 }}>FY10–11 → present</span>
        </div>
      </div>
    ),
    { ...size }
