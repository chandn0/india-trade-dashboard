...
function BrandTable({ title, icon: Icon, rows, accent }) {
  return <Paper component="section" sx={{ ...cardSx, p: 0, overflow: 'hidden' }}>
    ...
    {rows.map((row, index) => <Box key={row.brand} sx={{ display: 'grid', gridTemplateColumns: '1.25fr .65fr .7fr', gap: 1, alignItems: 'center', px: { xs: 2, md: 2.5 }, py: 1.45, borderBottom: index < rows.length - 1 ? '1px solid' : 0, borderColor: 'divider' }}>
...
export default function MobilityBrief() {
  return <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
    ...
    <Box sx={{ bgcolor: C.ink, color: '#fff' }}><Container maxWidth="xl" sx={{ pt: { xs: 4, md: 6 }, pb: { xs: 3.5, md: 5 } }}>
      ...
      <Typography variant="overline" sx={{ color: '#e7b48c' }}>India’s consumer brands · mobility</Typography>
      <Typography component="h1" sx={{ mt: .4, maxWidth: 920, fontFamily: 'var(--font-display)', fontSize: { xs: '2.45rem', md: '4.1rem' }, lineHeight: 1.02, letterSpacing: '-.035em' }}>What India rides. What India drives. What is actually made here?</Typography>
      <Typography sx={{ mt: 2, maxWidth: 760, color: 'rgba(255,255,255,.73)', fontSize: { xs: 15, md: 17 }, lineHeight: 1.65 }}>The brands behind India’s everyday mobility, with retail market share separated from the much harder question of model-level domestic value addition.</Typography>
    </Container></Box>
    <Container maxWidth="xl" sx={{ py: { xs: 3.5, md: 5 } }}><Stack spacing={{ xs: 4.5, md: 6 }}>
      ...
      <Box component="section"><Typography variant="overline" sx={{ color: C.orange }}>The brands people choose</Typography><Typography component="h2" variant="h4" sx={{ mt: .25, mb: .7 }}>The market is led by six familiar names in each category.</Typography><Typography sx={{ mb: 2.5, maxWidth: 800, color: 'text.secondary', fontSize: 14, lineHeight: 1.65 }}>Retail share below is FADA’s FY2024–25 registration data. It is deliberately shown separately from the newer industry-wide sales figures above.</Typography><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 2 }}><BrandTable title="Two-wheelers" icon={DirectionsBike} rows={mobility.twoWheelers} accent={C.teal} /><BrandTable title="Passenger vehicles" icon={DirectionsCar} rows={mobility.cars} accent={C.blue} /></Box><Box sx={{ mt: 1 }}><Box component="a" href={mobility.sources.retail.url} target="_blank" rel="noopener noreferrer" sx={linkSx}>{mobility.sources.retail.label} ↗</Box></Box></Box>
