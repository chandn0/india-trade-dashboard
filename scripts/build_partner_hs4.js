...
try {
  ...
  existingRows = [
    'hscode,hs2_grain,country,flow,fiscal_year,value_usd_mn,share_pct,evidence_note,source_url,source_date',
  ];
}
...
const targetHscodes = new Set([
  ...topDeficit.map((p) => p.hscode),
  ...topExport.map((p) => p.hscode),
]);
  ...
  for (const partner of partnerData.partners) {
    ...
    if (partner.imports && partner.imports.top_chapters) {
      ...
      if (impChapter) {
        ...
        newCsvRows.push(
          `"${hscode}","${hs2Grain}","${country}","import","${formattedFy}","${impChapter.value_usd_mn}","${sharePct.toFixed(1)}","HS-2 chapter ${hs2} grain; HS-4 specific partner breakdown not separately published by EIDB.","https://eidb.dgft.gov.in/","2026-06-10"`,
        );
      }
    }
    ...
    if (partner.exports && partner.exports.top_chapters) {
      ...
      if (expChapter) {
        ...
        newCsvRows.push(
          `"${hscode}","${hs2Grain}","${country}","export","${formattedFy}","${expChapter.value_usd_mn}","${sharePct.toFixed(1)}","HS-2 chapter ${hs2} grain; HS-4 specific partner breakdown not separately published by EIDB.","https://eidb.dgft.gov.in/","2026-06-10"`,
        );
      }
    }
  }
