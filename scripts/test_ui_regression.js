...
// Load core datasets
const stageMix = JSON.parse(fs.readFileSync(path.join(root, 'data/product_stage_mix.json'), 'utf8'));
const stageSummary = JSON.parse(fs.readFileSync(path.join(root, 'data/product_stage_summary.json'), 'utf8'));
const baselineSummary = JSON.parse(fs.readFileSync(path.join(root, 'tests/fixtures/baseline_totals.json'), 'utf8'));
...
describe('S05 Suite 1: Search and Hierarchical Filters (ProductExplorer)', () => {
  ...
  it('filters accurately by production stage', () => {
    const rawMaterials = filterAndSortProducts(importProducts, { stage: 'raw material', limit: 'all' });
  ...
  it('filters accurately by sector and HS-2 chapter', () => {
    ...
    const sectorProducts = filterAndSortProducts(importProducts, { sector: targetSector, limit: 'all' });
    ...
    const hs2Products = filterAndSortProducts(importProducts, { sector: targetSector, hs2: targetHs2, limit: 'all' });
  ...
  it('performs case-insensitive text search matching hscode, description, or attributionRationale', () => {
    ...
    assert.ok(codeMatch.some((p) => p.hscode === '8542'), 'Should find HS 8542 by exact code');
    ...
    for (const p of descMatch) {
      const match = p.hscode.includes('cotton') || p.description.toLowerCase().includes('cotton') || p.attributionReason.toLowerCase().includes('cotton');
...
describe('S05 Suite 2: Import/Export Flow Switching (ProductExplorer)', () => {
  it('switches between import and export value keys, ranks, and positive-flow subsets', () => {
    ...
    const importSorted = filterAndSortProducts(importActive, { flowName: 'Imports', sortBy: 'value', limit: 10 });
    const exportSorted = filterAndSortProducts(exportActive, { flowName: 'Exports', sortBy: 'value', limit: 10 });
...
describe('S05 Suite 3: Top-20 / 50 / 100 / All Slice and Sort Controls', () => {
  ...
  it('sorts correctly across all 4 modes (value, stage, code, net)', () => {
    ...
    for (let i = 0; i < byStage.length - 1; i += 1) {
      ...
      assert.ok(stageCmp <= 0 || (stageCmp === 0 && byStage[i].latestImportUsdMn >= byStage[i + 1].latestImportUsdMn));
    }
...
describe('S05 Suite 5: Scenario Arithmetic and Slider Boundaries (ScenarioModeller)', () => {
  it('calculates net impact accurately across standard and boundary slider inputs', () => {
    ...
    const zeroRes = calculateScenario(p, { substitution: 0, exportGrowth: 0, importedInputShare: 0, realisation: 0 });
    ...
    const zeroReal = calculateScenario(p, { substitution: 100, exportGrowth: 100, importedInputShare: 50, realisation: 0 });
    ...
    const fullRes = calculateScenario(p, { substitution: 100, exportGrowth: 100, importedInputShare: 0, realisation: 100 });
    ...
    const fullInput = calculateScenario(p, { substitution: 50, exportGrowth: 50, importedInputShare: 100, realisation: 100 });
    assert.ok(Math.abs(fullInput.netImpact) < 1e-9, `Expected ~0 net impact when input share is 100%, got ${fullInput.netImpact}`);

    // Realistic Default Outputs (Pinned for HS 8542)
    const realisticRes = calculateScenario(p, { substitution: 20, exportGrowth: 15, importedInputShare: 35, realisation: 70 });
    assert.ok(Math.abs(realisticRes.grossMovement - 6106.5275) < 1e-3);
    assert.ok(Math.abs(realisticRes.netImpact - 2778.4700) < 1e-3);
    
    // Realistic Default Outputs (Zero Export Product - HS 0103)
    const zeroExRes = calculateScenario(pZeroEx, { substitution: 20, exportGrowth: 15, importedInputShare: 35, realisation: 70 });
    ...
    const zeroImRes = calculateScenario(pZeroIm, { substitution: 20, exportGrowth: 15, importedInputShare: 35, realisation: 70 });
    assert.ok(Math.abs(zeroImRes.grossMovement - 21.2330) < 1e-3);
    assert.ok(Math.abs(zeroImRes.netImpact - 9.6610) < 1e-3);
...
describe('S05 Suite 6: Zero-Import and Zero-Export Products Handling', () => {
  it('handles zero-import lines safely with exportCoveragePct === null or 0', () => {
    ...
    for (const p of zeroImports) {
      assert.equal(p.exportCoveragePct, p.latestExportUsdMn > 0 ? null : 0, `Product HS ${p.hscode} with 0 imports should have correct coverage percentage logic`);
    }
  });

  it('handles zero-export lines safely with exportCoveragePct === 0 or null if both 0', () => {
    const zeroExports = stageMix.products.filter((p) => p.latestExportUsdMn === 0 && p.latestImportUsdMn > 0);
    for (const p of zeroExports) {
      assert.equal(p.exportCoveragePct, 0, `Product HS ${p.hscode} with 0 exports and positive imports should have 0% coverage`);
      assert.ok(p.netBalanceUsdMn < 0, `Product HS ${p.hscode} with only imports must have negative net balance`);
    }
  });
});
...
describe('S05 Suite 7: Desktop and Mobile Layout Data Contracts', () => {
  it('enforces COMPACT_BELOW breakpoint and checks chart geometry math', async () => {
    ...
    const sampleDeficitData = [
      { financial_year: '2021-22', exchange_rate_inr_per_usd: 74.5, cumulative_deficit_usd_mn: 200000 },
      { financial_year: '2022-23', exchange_rate_inr_per_usd: 80.2, cumulative_deficit_usd_mn: 450000 },
    ];
...
describe('S05 Suite 8: Main-Dashboard Summary vs Dedicated-Page Totals Reconciliation', () => {
  it('reconciles product_stage_summary.json exact totals against the static baseline_totals.json fixture', () => {
    for (const flowName of ['Imports', 'Exports']) {
      ...
      assert.ok(Math.abs(summaryFlow.totalUsdMn - baselineFlow.totalUsdMn) < 1e-4, `Total USD Mn drifted from approved baseline for ${flowName}`);
      assert.ok(Math.abs(summaryFlow.totalUsdMn - mixFlow.totalUsdMn) < 1e-4, `Total USD Mn mismatch between summary and mix for ${flowName}`);
      assert.equal(summaryFlow.productCount, baselineFlow.productCount, `Product count drifted from approved baseline for ${flowName}`);
      assert.equal(summaryFlow.productCount, mixFlow.productCount, `Product count mismatch for ${flowName}`);
      ...
      for (const stageInfo of summaryFlow.stages) {
        ...
        assert.equal(stageInfo.productCount, baselineStage.productCount, `Stage ${stageInfo.stage} product count drifted from baseline in ${flowName}`);
        assert.ok(Math.abs(stageInfo.valueUsdMn - baselineStage.valueUsdMn) < 1e-4, `Stage ${stageInfo.stage} value drifted from baseline in ${flowName}`);
        ...
        assert.equal(stageInfo.productCount, mixStage.productCount, `Stage ${stageInfo.stage} product count mismatch in ${flowName}`);
        assert.ok(Math.abs(stageInfo.valueUsdMn - mixStage.valueUsdMn) < 1e-4, `Stage ${stageInfo.stage} value mismatch in ${flowName}`);
      }
      ...
      assert.ok(Math.abs(aggregatedTotal - mixFlow.totalUsdMn) < 1e-4, `Sum of individual product lines (${aggregatedTotal}) does not match reported total (${mixFlow.totalUsdMn}) for ${flowName}`);
    }
